LootJS.lootTables((event) => {
  ...
  event.create("phis:manuscripts").createPool((pool) => {
...
LootJS.lootTables((event) => {
  ...
  for (const skeleton of skeletons) {
    event.modifyBlockTables(skeleton).createPool((pool) => {
      pool
        .addEntry(LootEntry.reference("phis:manuscripts").randomChance(0.1))
        .when((condition) => {
...
LootJS.modifiers((event) => {
  const magic_mobs = [
    ...
    "irons_spellbooks:entities/archevoker",
    "irons_spellbooks:entities/cryomancer",
    "irons_spellbooks:entities/necromancer",
    "irons_spellbooks:entities/pyromancer",
  ];
